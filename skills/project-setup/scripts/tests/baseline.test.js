...
test('initBaselines does NOT mark baselined when the threshold anchor is missing (retryable)', () => {
  // The generated config keeps its marker but the user removed the coverageThreshold
  // block, so applyCoverageFloor can't place a floor ('threshold anchor not found').
  // Restoring the anchor later must re-baseline, so the marker stays absent.
  const jestConfig = '// Generated by project-setup\nexport default { /* coverageThreshold removed */ };';
